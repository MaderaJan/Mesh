...
export async function runMesh() {
    ...
    const token: Credentials = await getToken(oauth2Client);
...
function persistToken(tokens: Credentials) {
    fileSystem.writeFileSync(TOKEN_FILE_PATH, JSON.stringify(tokens));
}

async function getDaySummary(tokens: Credentials, oauth2Client: OAuth2Client) {
    try {
...
function getAuthTokenFromStorage(): Credentials | null {
    try {
